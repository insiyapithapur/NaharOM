    ...
    if request.method == "POST":
        try:
            ...
            if not country_code or not mobile_number:
                return JsonResponse(
                    {"message": "countryCode and mobileNumber are required"}, status=400
                )
                ...
                if status_code == 200:
                    try:
                        ...
                        if userRole.role != user_role:
                            return JsonResponse(
                                {"message": "user role is not match"}, status=400
                            )
                        return JsonResponse(
                            {
                        ...
                        with transaction.atomic():
                            user = models.User.objects.create(
                                mobile=mobile_number, email="default@gmail.com"
                            )
...
def verifyStatusAPI(request, user):
    if request.method == "GET":
        try:
            if not user:
                return JsonResponse(
                    {"message": "userID should be there in url"}, status=400
                )
            ...
            if userRole.role == "Individual":
                Individual_Details_exist = models.IndividualDetails.objects.filter(
                    user_role=userRole
                ).exists()
                BankAcc_Details_exist = models.BankAccountDetails.objects.filter(
                    user_role=userRole
                ).exists()
                is_KYC = Individual_Details_exist
                is_BankDetailsExists = BankAcc_Details_exist
                ...
                Company_Details_exist = models.CompanyDetails.objects.filter(
                    user_role=userRole
                ).exists()
                BankAcc_Details_exist = models.BankAccountDetails.objects.filter(
                    user_role=userRole
                ).exists()
                is_KYC = Company_Details_exist
                is_BankDetailsExists = BankAcc_Details_exist
            ...
            if userRole.role == "Individual":
                ...
                if response.status_code == 200:
                    ...
                    alternate_phone_numbers = [
                        phone["phoneNumber"]
                        for phone in response_info["alternatePhone"]
                    ]
                    alternate_phone = next(
                        (
                            phone
                            for phone in alternate_phone_numbers
                            if phone != userRole.user.mobile
                        ),
                        None,
                    )

                    email = (
                        response_info["email"][0]["email"]
                        if response_info["email"]
                        else None
                    )
                    ...
                    pan_card_number = (
                        response_info["PAN"][0]["IdNumber"]
                        if response_info["PAN"]
                        else None
                    )

                    first_name = (
                        response_info["name"]["firstName"]
                        if "name" in response_info
                        and "firstName" in response_info["name"]
                        else None
                    )
                    last_name = (
                        response_info["name"]["lastName"]
                        if "name" in response_info
                        and "lastName" in response_info["name"]
                        else None
                    )

                    state = (
                        response_info["address"][0]["State"]
                        if response_info["address"]
                        else None
                    )
                    postal_code = (
                        response_info["address"][0]["Postal"]
                        if response_info["address"]
                        else None
                    )
