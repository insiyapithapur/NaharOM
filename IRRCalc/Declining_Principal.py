...
def Declining_calculate_Buyerprice_to_XIRR(
    ...
    while remaining_principal > 0:
        ...
        interest_payment = (365/12) * daily_interest_rate * remaining_principal  # Calculate interest payment
...
def Declining_calculate_Sellerprice_to_XIRR(
    fractional_unit_value,
...
def Declining_calculate_SellerXIRR_to_price(
    fractional_unit_value,
    loan_amount, # principal amount of the loan, e.g. 10,000,000
    num_fractions, # number of fractions for the loan, e.g. 20
    annual_interest_rate, # interest rate that buyer has to pay with invested amount
    ...
    difference = final_xirr - target_xirr
    print("\nCashflow details:")
    for date, amount in zip(dates, amounts):
        print(f"{date}: {amount:.2f}")
    print(f"{dates[-1]}: Sale Price: {additional_amount:.2f}")
    return dates, amounts, additional_amount, final_xirr, difference
