...
def get_next_schedule_date_seller(date, payment_frequency):
    if payment_frequency == "MONTHLY":
...
def Calculate_Seller_price(
    ...
    while current_date < end_date:
        ...
        prev_date = current_date
        current_date = get_next_schedule_date_seller(current_date, payment_frequency)
    ...
    if current_date != end_date:
        ...
        full_month_days = (get_next_schedule_date_seller(prev_date, payment_frequency) - prev_date).days
...
def calculate_cashflow_with_flexible_xirr(
    fractional_unit_value,
    loan_amount, # principal amount of the loan, e.g. 10,000,000
    num_fractions, # number of fractions for the loan, e.g. 20
    annual_interest_rate, # interest rate that buyer has to pay with invested amount
